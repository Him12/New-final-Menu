...
class PremiumMenu {
    ...
    // // ------------ EDITED: loadMenuData now uses MenuAPI.getMenu() when available ------------
    // async loadMenuData() {
    //     try {
    //         // Try to use MenuAPI.getMenu() if provided (menu-api.js)
    //         let data = null;
    //         if (window.MenuAPI && typeof window.MenuAPI.getMenu === 'function') {
    //             try {
    //                 data = await window.MenuAPI.getMenu();
    //             } catch (err) {
    //                 console.warn('MenuAPI.getMenu failed, falling back to local menu_data.json', err);
    //                 data = null;
    //             }
    //         }

    //         // If MenuAPI didn't return data, fallback to local JSON
    //         if (!data) {
    //             const response = await fetch('menu_data.json');
    //             if (!response.ok) throw new Error('Failed to load local menu data');
    //             data = await response.json();
    //         }

    //         // If data is an object with categories, proceed as before.
    //         // 🧩 Create “All” category dynamically — skip any specials
    //         const allItems = [];
    //         Object.keys(data).forEach(category => {
    //             const key = category.toLowerCase();
    ...
    async loadMenuData() {
        try {
            ...
            // ----------------- Normalization: accept many backend shapes -----------------
            // If backend returns a flat array of items, convert to categories format.
            if (Array.isArray(data)) {
                data = {
                    all: {
                        label: { en: "All", fr: "Tout", es: "Todo", it: "Tutti", jp: "すべて" },
                        items: data
                    }
                };
    ...
    */
    ...
    // ------------ EDITED: findItemById compares as strings to avoid number/string mismatch ------------
    findItemById(itemId) {
        for (const category of Object.values(this.MENU)) {
